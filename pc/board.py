...
def read_update_state():
    """R9: 读取本地更新状态（上次检查时间/上次更新时间/已知最新版），防同一版本反复弹提示。"""
    st = {"last_check": 0, "last_update": 0, "last_version": "", "pending_restart": False, "note": ""}
    try:
...
# R3: 限流相关数据结构
_RATE_LIMIT_WINDOW = 60                        # 限流时间窗口（秒）
_RATE_LIMIT_COUNT = 5                          # 时间窗口内最多消息数
_device_identities = {}                        # device_id -> {last_active: ts, identity_count: int, identities: set()}
_identity_ratelimit = {}                       # identity -> [(ts1, ts2, ...)] 滑动窗口
_blocklist_path = os.path.join(ROOT_DIR, "config", "device_blocklist.json")
_blocked_devices = set()                       # 已禁用的 device_id 集合
_device_id = ""                                # 本设备的唯一标识


def _load_device_blocklist():
    """R3: 加载设备黑名单（防止熊孩子频繁换身份发送消息）"""
    global _blocked_devices
    try:
        if os.path.exists(_blocklist_path):
            with open(_blocklist_path, "r", encoding="utf-8") as f:
                data = json.load(f)
                _blocked_devices = set(data.get("blocked_devices", []))
                print(f"[限流] 已加载 {len(_blocked_devices)} 个禁用设备")
    except Exception as e:
        print(f"[限流] 加载黑名单失败: {e}")
        _blocked_devices = set()


def _save_device_blocklist():
    """R3: 保存设备黑名单"""
    try:
        os.makedirs(os.path.dirname(_blocklist_path), exist_ok=True)
        data = {"blocked_devices": list(_blocked_devices)}
        with open(_blocklist_path, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
        print(f"[限流] 已保存 {len(_blocked_devices)} 个禁用设备")
    except Exception as e:
        print(f"[限流] 保存黑名单失败: {e}")


def _get_device_id():
    """R3: 获取或生成设备唯一标识（基于机器指纹）"""
    global _device_id
    if not _device_id:
        try:
            # 基于机器指纹生成稳定设备 ID
            import uuid
            machine_id = str(uuid.getnode())  # MAC 地址的最后 6 位（整数）
            import hashlib
            _device_id = hashlib.md5(f"hsboard-{machine_id}-{CLASS_ID}".encode()).hexdigest()[:16]
            print(f"[限流] 设备 ID: {_device_id}")
        except Exception:
            # 备用方案：使用随机 ID
            import random
            _device_id = f"dev_{random.randint(1000, 9999)}"
    
    # 加载黑名单
    _load_device_blocklist()
    
    # 检查是否在黑名单中
    if _device_id in _blocked_devices:
        print(f"[限流] 设备 {_device_id} 已被禁用")
        return None
    return _device_id


def _check_rate_limit(identity):
    """R3: 检查身份限流（60秒内>5条消息）和设备身份切换限流（1小时内换≥4身份）
    返回 (allowed: bool, reason: str, should_block_device: bool)"""
    now = time.time()
    
    # 检查设备是否已被禁用
    if _device_id in _blocked_devices:
        return False, "设备已被禁用", False
    
    # 1. 身份限流检查：60秒内最多5条消息
    if identity not in _identity_ratelimit:
        _identity_ratelimit[identity] = []
    
    # 清理过期的时间戳
    _identity_ratelimit[identity] = [ts for ts in _identity_ratelimit[identity] 
                                    if now - ts < _RATE_LIMIT_WINDOW]
    
    if len(_identity_ratelimit[identity]) >= _RATE_LIMIT_COUNT:
        print(f"[限流] 身份 {identity} 触发限流（{len(_identity_ratelimit[identity])}条/{_RATE_LIMIT_WINDOW}秒）")
        return False, f"身份 {identity} 发送过于频繁，请稍后再试", False
    
    # 2. 设备身份切换检查：1小时内换≥4个身份
    if _device_id not in _device_identities:
        _device_identities[_device_id] = {
            "last_active": now,
            "identity_count": 1,
            "identities": {identity}
        }
    else:
        device_data = _device_identities[_device_id]
        # 如果是新的身份
        if identity not in device_data["identities"]:
            # 检查1小时内是否切换了≥4个身份
            if now - device_data["last_active"] < 3600:  # 1小时
                device_data["identity_count"] += 1
                device_data["identities"].add(identity)
                
                if device_data["identity_count"] >= 4:
                    print(f"[限流] 设备 {_device_id} 1小时内切换了{device_data['identity_count']}个身份，触发禁用")
                    _blocked_devices.add(_device_id)
                    _save_device_blocklist()
                    return False, f"设备 {_device_id} 切换身份过于频繁，已被禁用", True
            else:
                # 超过1小时，重置计数
                device_data["last_active"] = now
                device_data["identity_count"] = 1
                device_data["identities"] = {identity}
    
    # 记录消息时间戳
    _identity_ratelimit[identity].append(now)
...
def _proxy_download_url(zip_url):
    """R7: 把 GitHub 直链套 gh-proxy v6 前缀（国内可达）；已是 proxy/非 GitHub 则原样返回。"""
    if "v6.gh-proxy.org/" in zip_url or "gh-proxy.com/" in zip_url or zip_url.startswith(GH_PROXY_PREFIX):
        return zip_url
...
def _download_with_fallback(zip_url, dest_path, timeout=60):
    """R7: 先走 GitHub 直连（raw），失败自动回退 gh-proxy（国内加速）。双保险。
    返回实际生效的下载源（'direct' / 'gh-proxy'），供日志排查。"""
    # R7: 先尝试 GitHub 直连（raw.githubusercontent.com）
    if "github.com" in zip_url:
        try:
            _download_bytes(zip_url, dest_path, timeout=timeout)
            return "direct"
        except Exception as e:
            print(f"[更新] GitHub 直连失败（{e.__class__.__name__}），回退 gh-proxy…")
            evt("UPDATE_DL", f"直连失败，回退 gh-proxy {zip_url}")
    ...
    if proxy_url != zip_url:
        try:
            ...
            print(f"[更新] gh-proxy 下载失败（{e.__class__.__name__}），按旧版启动")
            evt("UPDATE_DL", f"gh-proxy 失败 {proxy_url}")
    _download_bytes(zip_url, dest_path, timeout=timeout)
    return "direct"
...
def check_for_update_on_startup():
    """R7: 开机前更新检查：三源链式获取 update.json（raw→gh-proxy→家长端）。
    ...
    if "github.com" in url:
        try:
            ...
            source_used = "raw"
            print(f"[更新] 从 GitHub Raw 获取成功")
        except Exception as e:
            print(f"[更新] GitHub Raw 失败（{e.__class__.__name__}），尝试 gh-proxy…")
    
    # R7: 源2 - gh-proxy v6（回退）
    if remote is None:
        proxy_url = _proxy_download_url(url)
        if proxy_url != url:
            try:
                req = urllib.request.Request(proxy_url, headers=_h)
                with urllib.request.urlopen(req, timeout=10) as resp:
                    remote = json.loads(resp.read().decode("utf-8"))
                source_used = "gh-proxy"
                print(f"[更新] 从 gh-proxy 获取成功")
            except Exception as e:
                print(f"[更新] gh-proxy 失败（{e.__class__.__name__}），按当前版本启动")
    
    # R7: 源3 - 家长端站点（三源冗余）
    if remote is None and PARENT_SITE_URL:
        try:
            req = urllib.request.Request(PARENT_SITE_URL + "/update.json", headers=_h)
            with urllib.request.urlopen(req, timeout=10) as resp:
                remote = json.loads(resp.read().decode("utf-8"))
            source_used = "parent"
            print(f"[更新] 从家长端站点获取成功")
        except Exception as e:
            print(f"[更新] 家长端站点失败（{e.__class__.__name__}）")
    
    if remote is None:
        print(f"[更新] 所有源均失败，按当前版本启动")
        return False
    ...
    _ALLOWED_DOMAINS = ("github.com", "gh-proxy.com", "v6.gh-proxy.org")
    _zip_host = ""
    try:
    ...
    if not any(_zip_host.endswith(d) for d in _ALLOWED_DOMAINS):
        ...
        return False
    print(f"[更新] 发现新版本 {remote_ver}（当前 {VERSION}），来源 {source_used}：{remote.get('changelog','')}")
    try:
        need_restart = do_self_update(zip_url, remote.get("sha256"))
    except Exception as e:
        print(f"[更新] 自动更新失败（{e.__class__.__name__}），按旧版启动：{e}")
        need_restart = False
    # R9: 记录上次更新时间（成功触发更新程序时）
    update_fields = {"last_check": int(time.time()), "last_version": remote_ver,
                     "pending_restart": bool(need_restart), "note": remote.get("changelog", "")}
    if need_restart:
        update_fields["last_update"] = int(time.time())
    st.update(**update_fields)
    save_update_state(st)
    return need_restart
...
def _send_presence():
    ...
    try:
        ...
        print(f"[冲突] 发送 presence 失败: {e}")
    # 20秒后再次发送
    root.after(20000, _send_presence)
...
def on_message(client, userdata, msg):
    ...
    if msg.topic.startswith(ACK_PREFIX):
        ...
        try:
            ...
            if len(rids) > 200:
                del rids[:-200]
            # 设置 read_at 用于已读区排序（越近越靠上）
            read_at_str = a.get("readAt")
            if read_at_str:
                try:
                    from datetime import datetime
                    read_at_dt = datetime.fromisoformat(read_at_str)
                    read_at_ms = int(read_at_dt.timestamp() * 1000)
                    s = state.get(sid)
                    if s:
                        s["read_at"] = read_at_ms
                        # 如果卡片存在且有未读消息，且 ack 包含该 sid 的消息，则标记为已读
                        if s["unread"] > 0 and ids:
                            s["unread"] = 0
                except Exception:
                    pass
        except Exception:
...
# R3: 初始化设备 ID（必须在 MQTT 连接之前）
device_id = _get_device_id()
if not device_id:
    print("[限流] 设备已被禁用，无法启动")
    sys.exit(1)
...
def layout_cards():
    ...
    read_zone = sorted(
        ...
        key=lambda kv: (-(kv[1].get("read_at") or 0), kv[0]))   # 已读区降序：最近已读在上；同秒按 sid 稳定排序
    x1, x2 = Z(24), W - Z(24)
    mid = (x1 + x2) // 2
    gut = Z(5)                                 # 双列中缝
    y_c = Z(y0)
...
def render():
    ...
    if _interact_mode[0] != "resize":  # R8: 不再支持缩放，只检查是否为 resize 模式
        view = max(view, Z(400))        # 确保最小高度
    scroll_max[0] = max(0, content - view)
    if abs(view - H) > Z(6) and _interact_mode[0] != "resize":
        H = view                               # 拖拽缩放中不做高度回收（否则和用户拖拽打架）
    ...
    if _conflict_show[0]:
        banner_h = Z(36)
        rrect(Z(10), Z(70), W - Z(10), Z(70) + banner_h, Z(8),
              fill="#FF4444", outline="#FF8888", width=max(1, Z(1.5)))
        canvas.create_text(W // 2, Z(70) + banner_h // 2,
                           text="⚠ 检测到同班级另一台电脑正在运行",
                           font=("Microsoft YaHei", PF(10), "bold"), fill="white", justify="center")
    
    cards = [s for s in state.values() if s["messages"]]
    if not cards:
        canvas.create_text(W // 2, Z(140) + banner_h, text="暂时没有留言\n等家长的第一条消息吧",
...
def activate_card(sid):
    ...
    if sid in expanded:
        ...
        if s["unread"] > 0:
            s["unread"] = 0
            s["read_at"] = int(time.time() * 1000)  # 记录点击已读时刻，用于已读区排序（越近越靠上）
...
_interact_mode = ["none"]                      # "none" / "scroll" / "move" (R8: 移除 resize)
_move_state = [0, 0, 0, 0]                    # [x_root, y_root, win_x, win_y]
_rm_entered = [False]                         # move 已越过误触阈值 (R8: 只用于 move)
# _resize_state 已移除 (v1.0.9 不再支持缩放)
# _bg_stretch_due 已移除 (v1.0.9 不再支持缩放)


def hit_zone(event, w, h, r, hot):
    """R8: 只返回 None（内容区），不再支持 8 向缩放热区。v1.0.9 改为仅移动窗口。"""
    return None


# _cursor_for_zone 函数已删除（v1.0.9 不再支持缩放热区）


def _on_hover(event):
    """R8: 悬停时光标保持默认（不再支持缩放光标）。"""
...
def _bg_stretch_photo(tw, th):
    """拖拽缩放过程中把磨砂底图实时拉伸到新窗口尺寸。
    不拉伸的话新扩出的区域只露兜底色 PANEL（窗口右侧一大块深色矩形）；
    松手后 resample_background 会重新清晰采样，拖拽中的轻微拉伸模糊可接受。"""
    global WALL_BG
    src = WALL_BG_PIL[0]
    if src is None:
        return
    try:
        WALL_BG = ImageTk.PhotoImage(src.resize((max(1, int(tw)), max(1, int(th))), Image.BILINEAR))
    except Exception:
        pass


def _schedule_bg_render(nw, nh):
    """拖拽缩放 idle 节流：底图拉伸到新尺寸 + 内容按新宽度重绘（高频 Motion 合并为一次）。"""
    if _bg_stretch_due[0]:
        return
    _bg_stretch_due[0] = True

    def _do():
        _bg_stretch_due[0] = False
        _bg_stretch_photo(nw, max(nh, SAMPLE_H))
        try:
            render()
        except Exception:
            pass
...
def _on_press(event):
    """R8: 点击事件处理（不支持 resize，只支持标题栏移动、卡片展开、设置菜单）。"""
    _drag[0] = event.y
    _drag[1] = 0
    _drag[2] = False
    _invis_n[0] = 0
    ...
    # R8: 标题栏点击判定（y<75），触发移动模式；其他区域进入 scroll 模式
    if event.y < Z(75):
...
# R1: 移除图形化班级切换函数（防止熊孩子恶意切换）
# set_class 和 restart_process 函数已移除（v1.0.9）


menu = tk.Menu(root, tearoff=0)
# R1: 移除图形化班级切换菜单（防止熊孩子恶意切换）
...
def _show_about():
    ...
    messagebox.showinfo("关于班级留言板",
        ...
        f"适用班级：{CLASS_NAME}\n\n"
        f"━━━ 更新状态 ━━━\n"
        f"检查更新：{fmt(st.get('last_check', 0))}\n"
        f"上次更新：{fmt(st.get('last_update', 0))}")
...
# R1: 启动 presence 心跳（冲突检测）
if os.environ.get("HSBOARD_DEMO_STATE") not in ("1", "bottom"):
    root.after(5000, _send_presence)  # 延迟5秒启动心跳，避免连接未建立
